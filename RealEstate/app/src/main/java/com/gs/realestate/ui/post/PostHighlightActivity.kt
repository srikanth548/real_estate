...
class PostHighlightActivity : AppCompatActivity(), PaymentResultWithDataListener, ExternalWalletListener, DialogInterface.OnClickListener  {
    lateinit var imagesGrid: GridView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fusedLocationProviderClient =  LocationServices.getFusedLocationProviderClient(this@PostHighlightActivity)

        imagesGrid = binding.imagesgrid
        imagesUpload = ImageAdapter(imagesList,this)
        imagesGrid.setAdapter(imagesUpload);
...
//        highLightsAdapter.notifyDataSetChanged()
        ...
        binding.terms.setOnCheckedChangeListener { buttonView, isChecked ->
            if(isChecked) {
                showTerms()
            }
        }
    ...
    fun showTerms(){
        val terms = this@PostHighlightActivity.resources.getStringArray(com.gs.realestate.R.array.terms)
    ...
    private fun getPlaces(){
         ...
         */
         lifecycleScope.launchWhenCreated {
                try {
                    val latLngString = currentLocation.latitude.toString()+","+currentLocation.longitude.toString()
                    //String.format("%d,%d",currentLocation.latitude,currentLocation.longitude)
                    val response = apiInterface?.doPlaces("restaurants", latLngString, "restaurants", true, "distance", "AIzaSyD3Dh9p2T3Oghmdmdbbj7sQL50isgbefag")
                    response?.enqueue(object : retrofit2.Callback<PlacesPOJO.Root?> {
                            override fun onResponse(call: Call<PlacesPOJO.Root?>, response: Response<PlacesPOJO.Root?>) {
                                ...
                                if (response.isSuccessful) {
                                    ...
                                    Toast.makeText(applicationContext, "Error " + response.code() + " found.", Toast.LENGTH_SHORT)
                                        .show()
                                }
                            }
        ...
        * */
        ...
        try {
            var options = JSONObject()
                ...
                options.put("image","http://dev.rightmyproperty.in/static/media/right_my_property_logo.b2909bab8bfc2a254934.png")
